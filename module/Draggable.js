...
class Draggable {
  constructor(
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      const progressPercentage =
        (this.progress.currentLevel / this.progress.max) * 100; // Прогресс из 3 блоков
  ...
  onPointerMove(event) {
    ...
    document.querySelectorAll(".drop-zone").forEach((zone) => {
      ...
      const x_overlap = Math.max(
        0,
        Math.min(blockRect.right, zoneRect.right) -
          Math.max(blockRect.left, zoneRect.left),
      );
      const y_overlap = Math.max(
        0,
        Math.min(blockRect.bottom, zoneRect.bottom) -
          Math.max(blockRect.top, zoneRect.top),
      );
  ...
  onPointerUp(event) {
    ...
    if (
      bestZone &&
      parseInt(bestZone.dataset.index) === parseInt(this.element.dataset.index)
    ) {
      const zoneRect = bestZone.getBoundingClientRect();
      const offsetX =
        zoneRect.left +
        (zoneRect.width - blockRect.width) / 2 -
        this.originalRect.left;
      const offsetY =
        zoneRect.top +
        (zoneRect.height - blockRect.height) / 2 -
